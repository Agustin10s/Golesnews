...
interface ArticleData {
  id: number; title: string; excerpt: string; copete?: string; content: string;
...
export default function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function save(newStatus?: string) {
    ...
    const res = await fetch(`/api/cms/articles/${id}`, {
      ...
      body: JSON.stringify({
        title, copete, excerpt, category, status: finalStatus,
        content: contentRef.current, featured_image: featuredImage,
        tags: tags.split(',').map(t => t.trim()).filter(Boolean),
        section, subcategory, ...seo,
      }),
    });
  ...
  if (!loaded) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300, color: '#333', fontSize: 14 }}>
      Cargando artículo...
    </div>
  );
    ...
    return (
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24, padding: '12px 0', borderBottom: '1px solid #1a1a1a' }}>
          ...
          <span style={{ fontSize: 12, color: '#555' }}>Vista previa</span>
          <span style={{
            fontSize: 9, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase',
            padding: '3px 8px', borderRadius: 3,
            background: status === 'published' ? 'rgba(74,222,128,.08)' : 'rgba(250,204,21,.08)',
            color: status === 'published' ? '#4ade80' : '#facc15',
            border: `1px solid ${status === 'published' ? 'rgba(74,222,128,.2)' : 'rgba(250,204,21,.2)'}`,
          }}>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 'clamp(28px,4vw,50px)', fontWeight: 800, color: '#fff', lineHeight: 1.1, marginBottom: 12 }}>{title}</h1>
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <h1 style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 32, fontWeight: 800, color: '#fff', margin: 0, letterSpacing: -.5 }}>Editar nota</h1>
          <span style={{
            fontSize: 9, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase',
            padding: '3px 8px', borderRadius: 3,
            background: status === 'published' ? 'rgba(74,222,128,.08)' : 'rgba(250,204,21,.08)',
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {msg && <span style={{ fontSize: 12, color: msg === 'Guardado' ? '#4ade80' : '#e8353a', padding: '6px 12px', background: msg === 'Guardado' ? 'rgba(74,222,128,.08)' : 'rgba(232,53,58,.08)', border: `1px solid ${msg === 'Guardado' ? 'rgba(74,222,128,.2)' : 'rgba(232,53,58,.2)'}`, borderRadius: 4 }}>{msg}</span>}
          <button onClick={() => router.push('/cms/articles')} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 14px', background: 'none', border: '1px solid #2a2a2a', color: '#555', borderRadius: 4, fontSize: 12, cursor: 'pointer' }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15,18 9,12 15,6"/></svg>
            Volver
          </button>
          <button onClick={() => setPreview(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 14px', background: 'none', border: '1px solid #2a2a2a', color: '#777', borderRadius: 4, fontSize: 12, cursor: 'pointer' }}>
          ...
          {status !== 'published' ? (
            <button onClick={() => save('published')} disabled={saving}
              style={{ padding: '9px 18px', background: '#e8353a', color: '#fff', border: 'none', borderRadius: 4, fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              {saving ? 'Publicando...' : 'Publicar'}
            </button>
          ) : (
            <button onClick={() => save('draft')} disabled={saving}
              style={{ padding: '9px 16px', background: '#111', border: '1px solid #facc1540', color: '#facc15', borderRadius: 4, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
              Mover a borrador
            </button>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {(categories.length > 0 ? categories : [{ id: 0, slug: category, label: category, color: '#e8353a' }]).map(c => (
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={copete} onChange={e => setCopete(e.target.value)} rows={2}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#0a0a0a', border: '1px solid #1e1e1e', borderRadius: 5, padding: '16px' }}>
            ...
            <div
              ...
              onDrop={async e => { e.preventDefault(); setImgDragging(false); const file = e.dataTransfer.files[0]; if (file) await handleFeaturedImageFile(file); }}
              onClick={() => !featuredImage && imgInputRef.current?.click()}
              style={{
                border: `1px dashed ${imgDragging ? '#e8353a' : '#2a2a2a'}`,
                borderRadius: 4, background: imgDragging ? 'rgba(232,53,58,.05)' : 'transparent',
                cursor: featuredImage ? 'default' : 'pointer', transition: 'all .15s',
                overflow: 'hidden', position: 'relative', minHeight: featuredImage ? 0 : 90,
              }}
            >
              {featuredImage ? (
                // eslint-disable-next-line @next/next/no-img-element
          ...
          <div style={{ background: '#0a0a0a', border: '1px solid #1e1e1e', borderRadius: 5, padding: '16px' }}>
            ...
            <label style={{ ...FIELD_LABEL, marginBottom: 5 }}>Tags</label>
            <input value={tags} onChange={e => setTags(e.target.value)} placeholder="messi, river..."
